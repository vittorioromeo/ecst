...
namespace example
{
    ...
    namespace system
    {
        ...
        struct render_colored_circle
        {
            ...
            template <typename TRenderTarget, typename TF>
            void render_on(TRenderTarget& rt, TF&& f)
            {
                // rt.setActive(true);
                f(rt);
                // rt.setActive(false);
                ...
                data.for_entities([this, &data, &va](auto eid)
                    {
                        ...
                        auto mk_triangle = [this, &va, &data, &p, &c, &cs](
    ...
    constexpr auto entity_count = ecst::sz_v<500000>;
...
#if defined(ENABLE_RENDERING)
                    // std::cout << "final rendering A\n";
                    proxy.template system<s::render_colored_circle>().render_on(
                        this->window(), [&proxy](auto& rt)
                        {
                            proxy.template for_system_outputs<
                                s::render_colored_circle>([&rt](auto&, auto& va)
                                {
