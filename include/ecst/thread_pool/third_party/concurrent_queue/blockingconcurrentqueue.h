        ...
        namespace mpmc_sema
        {
            ...
            public:
                Semaphore(int initialCount = 0)
                {
                    ...
                    m_hSema = CreateSemaphoreW(
                        nullptr, initialCount, maxLong, nullptr);
                }
                ...
                bool timed_wait(std::uint64_t usecs)
                {
                    ...
                    return WaitForSingleObject(
                               m_hSema, (unsigned long)(usecs / 1000)) !=
                           RC_WAIT_TIMEOUT;
                }
            ...
            public:
                ...
                {
                    assert(initialCount >= 0);
                    semaphore_create(mach_task_self(), &m_sema,
...
#error Unsupported platform! (No semaphore wrapper available)
            ...
            private:
                ...
                bool waitWithPartialSpinning(std::int64_t timeout_usecs = -1)
                {
                    ...
                    // If we lower it to 1000, testBenaphore becomes 15x slower
                    // on my Core i7-5930K Windows PC,
                    ...
                    while(--spin >= 0)
                    {
                        ...
                        if((oldCount > 0) &&
                            m_count.compare_exchange_strong(oldCount,
                                oldCount - 1, std::memory_order_acquire,
                                std::memory_order_relaxed))
                            return true;
                        std::atomic_signal_fence(
                            std::memory_order_acquire); // Prevent the compiler
                                                        // from collapsing the
                                                        // loop.
                    }
                    ...
                    if(m_sema.timed_wait((std::uint64_t)timeout_usecs))
                        return true;
                    // At this point, we've timed out waiting for the semaphore,
                    // but the
                    // count is still decremented indicating we may still be
                    // waiting on
                    // it. So we have to re-adjust the count, but only if the
                    // semaphore
                    // wasn't signaled enough times for us too since then. If it
                    // was, we
                    ...
                    while(true)
                    {
                        ...
                        if(oldCount < 0 &&
                            m_count.compare_exchange_strong(oldCount,
                                oldCount + 1, std::memory_order_relaxed,
                    ...
                    ssize_t max, std::int64_t timeout_usecs = -1)
                {
                    ...
                    while(--spin >= 0)
                    {
                        ...
                        {
                            ssize_t newCount =
                                oldCount > max ? oldCount - max : 0;
                            if(m_count.compare_exchange_strong(oldCount,
                                   newCount, std::memory_order_acquire,
                                   std::memory_order_relaxed))
                    ...
                    if(oldCount <= 0)
                    {
                        if(timeout_usecs < 0)
                            m_sema.wait();
                        else if(!m_sema.timed_wait(
                                    (std::uint64_t)timeout_usecs))
                        {
                            while(true)
                            {
                                oldCount =
            ...
            public:
                ...
                ssize_t waitMany(ssize_t max, std::int64_t timeout_usecs)
                {
                    ...
                    if(result == 0 && max > 0)
                        result =
                            waitManyWithPartialSpinning(max, timeout_usecs);
                    return result;
                }
    ...
    // This is a blocking version of the queue. It has an almost identical
    // interface to
    // the normal non-blocking version, with the addition of various
    // wait_dequeue() methods
    ...
    public:
        ...
        static const size_t MAX_SUBQUEUE_SIZE =
            ConcurrentQueue::MAX_SUBQUEUE_SIZE;
    ...
    public:
        ...
        explicit BlockingConcurrentQueue(size_t capacity = 6 * BLOCK_SIZE)
            : inner(capacity), sema(create<LightweightSemaphore>(),
                                   &BlockingConcurrentQueue::template destroy<
                                        LightweightSemaphore>)
        {
            assert(reinterpret_cast<ConcurrentQueue*>(
                       (BlockingConcurrentQueue*)1) ==
                       &((BlockingConcurrentQueue*)1)->inner &&
                   "BlockingConcurrentQueue must have ConcurrentQueue as its "
                   "first member");
            if(!sema)
            {
        ...
        BlockingConcurrentQueue(
            BlockingConcurrentQueue const&) MOODYCAMEL_DELETE_FUNCTION;
        BlockingConcurrentQueue& operator=(
            BlockingConcurrentQueue const&) MOODYCAMEL_DELETE_FUNCTION;
        ...
        inline BlockingConcurrentQueue& operator=(
            BlockingConcurrentQueue&& other) MOODYCAMEL_NOEXCEPT
        {
    ...
    public:
        ...
        {
            return wait_dequeue_timed(item,
        ...
        inline size_t wait_dequeue_bulk_timed(
            It itemFirst, size_t max, std::int64_t timeout_usecs)
        {
            size_t count = 0;
            max = (size_t)sema->waitMany(
