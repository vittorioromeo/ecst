...
namespace ecst::mp::list
{
    ...
    // TODO
    template <typename X, template <typename...> typename T, typename... Ts>
    constexpr auto index_of2(T<Ts...>)
    {
        int i = 0;
        return ((std::is_same_v<X, Ts> ? false : ++i) && ...) ? -1 : i;
    }

    // Returns the index of `x` in `l`.
    template <typename TList, typename T>
    constexpr auto index_of(TList, T &&) noexcept
    {
        return bh::size_c<index_of2<std::decay_t<T>>(TList{})>;
    }
