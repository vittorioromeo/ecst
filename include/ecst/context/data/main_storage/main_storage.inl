...
namespace ecst::context::impl
{
    ...
    auto& ECST_PURE_FN main_storage<TSettings>::metadata(entity_id eid) noexcept
    {
        return _entity_storage.get_metadata(eid);
    }

    template <typename TSettings>
    auto& ECST_PURE_FN main_storage<TSettings>::metadata(entity_id eid) const
        noexcept
    {
        return _entity_storage.get_metadata(eid);
    }
    ...
    template <typename TSettings>
    auto main_storage<TSettings>::valid_handle(const handle& h) const noexcept
    {
